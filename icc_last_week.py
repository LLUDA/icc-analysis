#!/usr/bin/env python3
"""
ICC Live Backtest - Last Week's Market Moves
=============================================
"""
...
class ICCLastWeek:
    """Test ICC method on last week's market data"""
    ...
    def run_backtest(self):
        ...
        for pair in self.pairs:
            ...
            try:
                # Load 2 weeks of data for context
                end_date = current_time.strftime("%Y-%m-%d")
                start_date = (current_time - timedelta(days=14)).strftime("%Y-%m-%d")
                ...
                if len(df_4h) < 30 or len(df_1h) < 100:
                    print(f"  Not enough data ({len(df_4h)} 4H, {len(df_1h)} 1H)")
                    continue

                print(
                    f"  Data: {df_1h.index[0].strftime('%Y-%m-%d')} to {df_1h.index[-1].strftime('%Y-%m-%d')}"
                )
                ...
                for idx, level in swings_low_1h:
                    if idx < 10 or idx > len(closes_1h) - 20:
                        continue
                ...
                if setups:
                    ...
                    for i, s in enumerate(setups, 1):
                        ...
                        print(f"     SL: {s['sl']:.5f} | TP: {s['tp']:.5f}")
                        print(
                            f"     RR: {s['rr']:.2f}:1 | Correction: {s['correction_pct']:.0f}%"
                        )
                        print(f"     Result: {s['outcome']}")
        ...
        if all_setups:
            ...
            print("\n  No setups found in last week")


if __name__ == "__main__":
    ICCLastWeek().run_backtest()
